...
if ($nv_Request->isset_request('change_active','post,get')){
    $id = $nv_Request->get_int('id','post,get','');
    $sql = "SELECT id, active FROM `nv4_samples` WHERE id = " .$id;
    $result = $db->query($sql);
    if ($row=$result->fetch()){
        $active = $row['active'] == 1 ? 0 : 1;
        $exe = $db->query("UPDATE `nv4_samples` SET active = " . $active ." WHERE id = ".$id);
        if ($exe){
            echo $active;
            die('OK');
        }
...
if ($nv_Request->isset_request('change_weight','post,get')){
    ...
    if ($id>0 && $new_weight>0){
        ...
        while ($row = $result->fetch()){
            ...
            $exe = $db->query("UPDATE `nv4_samples` SET weight = " . $weight ." WHERE id = ".$row['id']);
        }

        $exe = $db->query("UPDATE `nv4_samples` SET weight = " . $new_weight ." WHERE id = ".$id);
    }
}
...
if ($nv_Request->isset_request('action','post,get')){
    $id = $nv_Request->get_int('id','post,get',0);
    $checkess = $nv_Request->get_title('checkess','post,get',0);

    if($id>0 && $checkess==md5($id.NV_CHECK_SESSION)){
