...
$base_url = NV_BASE_SITEURL . "index.php?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&amp;" . NV_NAME_VARIABLE . "=" . $module_name . "&amp;q=" . urlencode( $key ) . "&amp;catid=" . $catid . "&amp;topicid=" . $topicid;

$where = "";
$tbl_src = " LEFT JOIN `" . NV_PREFIXLANG . "_" . $module_data . "_bodytext` as tb2 ON ( tb1.id =  tb2.id )";

if( ! empty( $key ) and $key != $lang_module['search_enter'] )
{
	$dbkey = $db->dblikeescape( $key );
	
	$where = " AND ( tb1.title LIKE '%" . $dbkey . "%' OR tb1.keywords LIKE '%" . $dbkey . "%' ";
	$where .= " OR tb1.author LIKE '%" . $dbkey . "%' OR tb2.bodytext LIKE '%" . $dbkey . "%')";
}

if( $topicid > 0 )
{
	$where .= " AND tb1.topicid=" . $topicid;
}
...
$array_content = array();
$url_link = NV_BASE_SITEURL . "index.php?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&amp;" . NV_NAME_VARIABLE . "=" . $module_name_search . "&" . NV_OP_VARIABLE . "=";
...
while( list( $id, $title, $alias, $catid, $hometext, $author, $publtime, $homeimgfile, $homeimgthumb, $sourceid ) = $db->sql_fetchrow( $result ) )
{
	...
	if( $array_img[0] != "" and file_exists( NV_ROOTDIR . '/' . NV_FILES_DIR . '/' . $module_name_search . '/' . $array_img[0] ) )
	{
		$img_src = NV_BASE_SITEURL . NV_FILES_DIR . '/' . $module_name_search . '/' . $array_img[0];
	}
	elseif( $homeimgfile != "" and file_exists( NV_UPLOADS_REAL_DIR . '/' . $module_name_search . '/' . $homeimgfile ) )
	{
		$img_src = NV_BASE_SITEURL . NV_UPLOADS_DIR . '/' . $module_name_search . '/' . $homeimgfile;
	}
...
$contents .= call_user_func( "search_result_theme", $key, $numRecord, $per_pages, $pages, $array_content, $url_link, $catid, $base_url );

if( empty( $key ) or $key == $lang_module['search_enter'] )
{
