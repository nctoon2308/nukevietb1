...
if( ! defined( 'NV_IS_MOD_THUOCLOBAN' ) )
{
    die( 'Stop!!!' );
}
...
function create_image()
{
    ...
    if( $trimStart >= $width ) $page = $trimStart / $width;
    else  $page = 0;

    if( $page > 0 ) $am = ( $page * $width ) % 5220;

    $image = ImageCreate( $width, $height );

    $image1 = imagecreatefrompng( $link_images . "/thuoc522.png" ); //file ảnh chia khung tốt, xấu
    $image2 = imagecreatefrompng( $link_images . "/thuoc.png" ); //file ảnh chia thước (1cm)
    ...
    $font_file = $link_images . '/fonts/arial.ttf';

    if( $page > 0 ) imagefttext( $image, 8, 0, -9, 25, $black, $font_file, ( ( $page * $width ) / imagesx( $image2 ) ) . ' cm' );
    else  imagefttext( $image, 8, 0, 1, 25, $black, $font_file, 'cm' );
